...
def main():
    ...
    if args.world_size > 1:
        # if device != "npu" and args.rank == 0:
        #     print("Error: distributed training is only available with npu device", file=sys.stderr)
        #     sys.exit(1)
        torch_npu.npu.set_device(args.rank % torch_npu.npu.device_count())
        distributed.init_process_group(backend="hccl",
    ...
    if args.augment:
        augment = nn.Sequential(FlipSign(), 
                                FlipChannels(), 
